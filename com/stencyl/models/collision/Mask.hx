...
class Mask
{
	...
	public inline function projectMask(axis:Point, collisionInfo:CollisionInfo):Void
	{
		...
		cur = -parent.currOffset.x * axis.x - parent.currOffset.y * axis.y;
		if (cur < min)
			min = cur;
		if (cur > max)
			max = cur;

		cur = (-parent.currOffset.x + parent.cacheWidth) * axis.x - parent.currOffset.y * axis.y;
		if (cur < min)
			min = cur;
		if (cur > max)
			max = cur;

		cur = -parent.currOffset.x * axis.x + (-parent.currOffset.y + parent.cacheHeight) * axis.y;
		if (cur < min)
			min = cur;
		if (cur > max)
			max = cur;

		cur = (-parent.currOffset.x + parent.cacheWidth) * axis.x + (-parent.currOffset.y + parent.cacheHeight)* axis.y;
		if (cur < min)
			min = cur;
		if (cur > max)
			max = cur;
