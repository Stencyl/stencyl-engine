...
public class b2Segment
{
	...
	*/
	...
	public function TestSegment(lambda:Array, // float pointer
		...
		var k_slop:Float = 100.0 * Number.MIN_VALUE;
	...
	*/
	public function ExtendForward(aabb:B2AABB) : Void{
		...
		var lambda:Float = Math.min(	dX>0?(aabb.upperBound.x-p1.x)/dX: dX<0?(aabb.lowerBound.x-p1.x)/dX:Float.POSITIVE_INFINITY,
										dY>0?(aabb.upperBound.y-p1.y)/dY: dY<0?(aabb.lowerBound.y-p1.y)/dY:Float.POSITIVE_INFINITY);
	...
	*/
	public function ExtendBackward(aabb:B2AABB) : Void{
		...
		var lambda:Float = Math.min(	dX>0?(aabb.upperBound.x-p2.x)/dX: dX<0?(aabb.lowerBound.x-p2.x)/dX:Float.POSITIVE_INFINITY,
										dY>0?(aabb.upperBound.y-p2.y)/dY: dY<0?(aabb.lowerBound.y-p2.y)/dY:Float.POSITIVE_INFINITY);
